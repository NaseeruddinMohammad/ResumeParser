...
public class Utils {
	...
	public JSONObject getResultJSON(JSONObject obj) {
		...
		if (getSkills(obj) != null) {
			...
			logger.info("Skills not found in the resume");
		}
		if (getWork(obj) != null) {
			resultJSON.put("work", getWork(obj));
		} else {
			logger.info("Work not found in the resume");
		}
		if (getInterests(obj) != null) {
			resultJSON.put("interests", getInterests(obj));
		} else {
			logger.info("Interests not found in the resume");
		}
		if (getEducation(obj) != null) {
			resultJSON.put("education", getEducation(obj));
		} else {
			logger.info("Education not found in the resume");
		}
		return resultJSON;
	}
	...
	public JSONObject getBasics(JSONObject obj) {
		if (obj.containsKey("basics")) {
			...
			logger.info("Basic Information not found in the resume");
		}
		if (obj.containsKey("summary")) {
			...
			logger.info("Summary not found in the resume");
		}
		return basicJSON;
	}
	...
	public JSONArray getWork(JSONObject obj) {
		...
		if (obj.containsKey("work_experience")) {
			...
			for (int i = 0; i < arr.size(); i++) {
				...
				for (String key : works) {
					if ((json.containsKey(key))) {
						...
						logger.info("Work experience detail " + key + " not found");
					}
				}
				if (!work.isEmpty()) {
					workJSON.add(work);
			...
			logger.info("Work experience data not found in the resume");
		}
